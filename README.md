...
视觉风格是 **像素风 + 液态玻璃质感 + 超大圆角 + 丰富过渡动画**：用像素字体（Press Start 2P / Silkscreen）、玻璃拟态面板、扫描线/噪点叠加，配 13 套可一键切换的主题（切换时整页有过渡动画，封面墙会跟着变调）。
...
- **13 套主题 + 液态玻璃**：运行时切换 CSS 变量，带过渡动画；偏好持久化到 localStorage。
...
- 模拟器内核（jsnes/nostalgist）、识别管线、fflate 均不进首屏。
